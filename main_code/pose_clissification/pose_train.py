...
train_dataloader = DataLoader(dataset=train_dataset, batch_size=2, shuffle=True, num_workers=4,drop_last=True)
val_dataloader = DataLoader(dataset=val_dataset, batch_size=2, shuffle=False, num_workers=4, drop_last=True)
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        if current_epoch_loss < best_epoch_loss:
            ...
            print("当前epoch:{}的loss最小".format(epoch))
            torch.save(model, "best_in_train.pt")  # 验证集上得到的best模型未必就是最好的，这里保存训练集上loss最小的模型
        ...
        if epoch % 2 == 0:
            ...
            if accuracy > best_accuracy:
                ...
                torch.save(model, 'best_in_val.pt')#模型较小，直接保存完整模型，加载较方便，torch.load("best_in_val.pt")
